...
import net.clankerjockey.mod.ClankerJockeyMod;
 ...
 * whose AI is entirely controlled by the ClankerJockey agent (follow player /
...
public class CompanionEntity extends VillagerEntity {
    ...
    public static CompanionEntity create(World world) {
        return ClankerJockeyMod.COMPANION_TYPE.create(world);
    }
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        if (followTargetName != null) nbt.putString("ClankerJockeyFollow", followTargetName);
        nbt.putDouble("ClankerJockeyFollowDistance", followDistance);
        if (moveTarget != null) {
            nbt.putIntArray("ClankerJockeyMoveTarget",
                    new int[]{moveTarget.getX(), moveTarget.getY(), moveTarget.getZ()});
            nbt.putDouble("ClankerJockeyMoveCloseness", moveCloseness);
        }
    }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("ClankerJockeyFollow")) followTargetName = nbt.getString("ClankerJockeyFollow");
        followDistance = nbt.contains("ClankerJockeyFollowDistance") ? nbt.getDouble("ClankerJockeyFollowDistance") : 4.0D;
        if (nbt.contains("ClankerJockeyMoveTarget")) {
            int[] p = nbt.getIntArray("ClankerJockeyMoveTarget");
            if (p.length == 3) moveTarget = new BlockPos(p[0], p[1], p[2]);
            moveCloseness = nbt.contains("ClankerJockeyMoveCloseness") ? nbt.getDouble("ClankerJockeyMoveCloseness") : 2.0D;
        }
    }
}
