 ...
 *
 * <p>Delegated to {@link ClankerJockeyMod#startVoiceLoop()}/{@link
 * ClankerJockeyMod#stopVoiceLoop()}: an unavailable microphone or STT bundle never
...
public final class VoiceKeybind {
    ...
    public static final KeyMapping TOGGLE_VOICE = new KeyMapping(
            "key.clankerjockey.toggleVoice",
            InputConstants.Type.KEYSYM,
            GLFW.GLFW_KEY_Y,
            "category.clankerjockey");

    private VoiceKeybind() {
    }

    @Mod.EventBusSubscriber(modid = ClankerJockeyMod.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.MOD)
    ...
    @Mod.EventBusSubscriber(modid = ClankerJockeyMod.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
    ...
    private static void toggleVoiceLoop() {
        if (ClankerJockeyMod.isVoiceLoopActive()) {
            ClankerJockeyMod.stopVoiceLoop();
            ClankerJockeyAgent.announceVoice("Voice chat off — talk to me in chat instead.");
        } else if (ClankerJockeyMod.startVoiceLoop()) {
            ClankerJockeyAgent.announceVoice("Listening...");
        } else {
            ClankerJockeyAgent.announceVoice("No microphone or voice input available — type to me instead.");
        }
    }
}
