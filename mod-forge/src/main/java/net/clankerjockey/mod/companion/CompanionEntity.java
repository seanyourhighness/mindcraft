 ...
 * entirely controlled by the ClankerJockey agent (follow player / move to
...
public class CompanionEntity extends Villager {
    ...
    public static CompanionEntity create(Level level) {
        return ClankerJockeyModCompanion.COMPANION_TYPE.get().create(level);
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        if (followTargetName != null) tag.putString("ClankerJockeyFollow", followTargetName);
        tag.putDouble("ClankerJockeyFollowDistance", followDistance);
        if (moveTarget != null) {
            tag.putIntArray("ClankerJockeyMoveTarget", new int[]{moveTarget.getX(), moveTarget.getY(), moveTarget.getZ()});
            tag.putDouble("ClankerJockeyMoveCloseness", moveCloseness);
        }
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("ClankerJockeyFollow")) followTargetName = tag.getString("ClankerJockeyFollow");
        followDistance = tag.contains("ClankerJockeyFollowDistance") ? tag.getDouble("ClankerJockeyFollowDistance") : 4.0D;
        if (tag.contains("ClankerJockeyMoveTarget")) {
            int[] p = tag.getIntArray("ClankerJockeyMoveTarget");
            if (p.length == 3) moveTarget = new BlockPos(p[0], p[1], p[2]);
            moveCloseness = tag.contains("ClankerJockeyMoveCloseness") ? tag.getDouble("ClankerJockeyMoveCloseness") : 2.0D;
        }
    }
}
